...
import { ArticleEditData, ArticleFormData } from "@/lib/types/article";
...
export default function EditForm({ article, categories, tags }: EditFormProps) {
  const router = useRouter();
  const { t } = useLanguage();

  const [titleId, setTitleId] = useState(article.translations.id.title || "");
  const [titleEn, setTitleEn] = useState(article.translations.en.title || "");
  const [descriptionId, setDescriptionId] = useState(article.translations.id.description || "");
  const [descriptionEn, setDescriptionEn] = useState(article.translations.en.description || "");
  const [contentId, setContentId] = useState(article.translations.id.content || "");
  const [contentEn, setContentEn] = useState(article.translations.en.content || "");
  const [thumbnail, setThumbnail] = useState<File | null>(null);
  const [status, setStatus] = useState<"draft" | "published">(article.status || "draft");
  const [readingTime, setReadingTime] = useState<number>(article.reading_time || 5);
  const [selectedCategories, setSelectedCategories] = useState<number[]>(article.categories?.map((category) => category.id) || []);
  const [selectedTags, setSelectedTags] = useState<number[]>(article.tags?.map((tag) => tag.id) || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.postFormData<ArticleFormData>(`/api/articles/${article.id}`, formData);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Field>
          ...
          <FieldDescription>
            ...
            {thumbnail ? (
              ...
              <span>
                {t("form.article.description.thumbnail.3")} {article.thumbnail_url}
              </span>
            )}
          </FieldDescription>
        ...
        <Field>
          ...
          <MultiSelect
            ...
            placeholder={t("form.article.placeholder.category")}
          />
        ...
        <Field>
          ...
          <MultiSelect options={tags.map((tag) => ({ id: tag.id, label: tag.name }))} selected={selectedTags} onChange={setSelectedTags} placeholder={t("form.article.placeholder.tag")} />
