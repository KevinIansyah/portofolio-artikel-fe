...
interface DataTableProps {
  initialData: Paginator<Skill> | null;
}
...
export function DataTable({ initialData }: DataTableProps) {
  ...
  React.useEffect(() => {
    ...
    const newUrl = params.toString() ? `?${params.toString()}` : "/dashboard/skill";
  ...
  const buildApiUrl = React.useCallback(() => {
    ...
    return `/api/skills/paginated?${params.toString()}`;
  }, [currentPage, perPage, debouncedSearch]);

  // Fetch data with SWR
  const { data, error, isLoading } = useSWR<Paginator<Skill>>([buildApiUrl(), language], ([url]) => apiClient.get(url), {
    ...
    <div className="w-full space-y-4">
        ...
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="bg-input/30">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          Showing {meta?.from || 0} to {meta?.to || 0} of {meta?.total || 0} skills
        </div>
