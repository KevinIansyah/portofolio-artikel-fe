...
interface DataTableProps {
  initialData: Paginator<Tag> | null;
}
...
export function DataTable({ initialData }: DataTableProps) {
  ...
  React.useEffect(() => {
    ...
    const newUrl = params.toString() ? `?${params.toString()}` : "/dashboard/tag";
  ...
  const buildApiUrl = React.useCallback(() => {
    ...
    return `/api/tags/paginated?${params.toString()}`;
  }, [currentPage, perPage, debouncedSearch]);

  // Fetch data with SWR
  const { data, error, isLoading } = useSWR<Paginator<Tag>>([buildApiUrl(), language], ([url]) => apiClient.get(url), {
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          Showing {meta?.from || 0} to {meta?.to || 0} of {meta?.total || 0} tags
        </div>
